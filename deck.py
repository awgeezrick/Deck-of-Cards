...
class Deck:
	...
	def _deal(self, num):
		...
		actual = min([count,num]) # Going to remove {actual} cards
		cards = self.cards[-actual:] #removing a slice starting from the last object to the "actual"
		self.cards = self.cards[:-actual] #aktualizujemy self.cards to slice'a od początku do "actual"
		return cards
